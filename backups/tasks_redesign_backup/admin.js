...
let curEmpPage = 1;
let empRowsPerPage = 50;

function loadEmployees(filterText = "") {
    const tbody = document.getElementById('employees-table-body');
    const paginationEl = document.getElementById('emp-pagination');
    if (!tbody) return;
    ...
    if (filterText) {
        ...
        filteredData = employees.filter(emp => 
            (emp.name && emp.name.toLowerCase().includes(txt)) || 
            (emp.iqama && emp.iqama.includes(txt)) || 
            (emp.branch && emp.branch.toLowerCase().includes(txt))
        );
        // Reset to page 1 on search
        if(curEmpPage !== 1 && !window.keepPage) curEmpPage = 1;
    }
    window.keepPage = false; // Reset flag

    // Pagination Logic
    const totalRows = filteredData.length;
    const totalPages = Math.ceil(totalRows / empRowsPerPage) || 1;
    if (curEmpPage > totalPages) curEmpPage = totalPages;
    if (curEmpPage < 1) curEmpPage = 1;

    const startIndex = (curEmpPage - 1) * empRowsPerPage;
    const endIndex = Math.min(startIndex + empRowsPerPage, totalRows);
        ...
        paginationEl.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                <div style="display:flex; align-items:center; gap:15px;">
                    <span style="color:#94a3b8; font-size:0.9rem;">إجمالي: <span style="color:#fff; font-weight:bold;">${totalRows}</span></span>
                    <select onchange="changeEmpRows(this.value)" class="form-control" style="width:auto; padding:4px 8px; height:30px; font-size:0.85rem;">
                        <option value="50" ${empRowsPerPage == 50 ? 'selected' : ''}>50 / صفحة</option>
                        <option value="100" ${empRowsPerPage == 100 ? 'selected' : ''}>100 / صفحة</option>
                        <option value="200" ${empRowsPerPage == 200 ? 'selected' : ''}>200 / صفحة</option>
                    </select>
                </div>
                <div style="display:flex; align-items:center; gap:10px;">
                    <button class="btn btn-secondary" ${curEmpPage === 1 ? 'disabled' : ''} onclick="changeEmpPage(-1)" style="padding:4px 10px;">&lt; السابق</button>
                    <span style="color:#fff; font-size:0.9rem;">صفحة ${curEmpPage} من ${totalPages}</span>
                    <button class="btn btn-secondary" ${curEmpPage === totalPages ? 'disabled' : ''} onclick="changeEmpPage(1)" style="padding:4px 10px;">التالي &gt;</button>
                </div>
            </div>
        `;
    }
...
function changeEmpPage(delta) {
    window.keepPage = true;
    curEmpPage += delta;
    const searchVal = document.getElementById('emp-search')?.value || "";
    loadEmployees(searchVal);
}

function changeEmpRows(val) {
    empRowsPerPage = parseInt(val);
    curEmpPage = 1;
    const searchVal = document.getElementById('emp-search')?.value || "";
    loadEmployees(searchVal);
}
...
function renderViolationsEditor() {
    ...
    const totalPages = Math.ceil(filtered.length / vioRowsPerPage);
    if (window.vioCurrentPage > totalPages) window.vioCurrentPage = 1;
    if (window.vioCurrentPage < 1) window.vioCurrentPage = 1;
    
    const start = (window.vioCurrentPage - 1) * vioRowsPerPage;
    const displayRows = filtered.slice(start, start + vioRowsPerPage);
    ...
    if(pagination) {
        pagination.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                <div style="display:flex; align-items:center; gap:15px;">
                    <span style="color:#94a3b8; font-size:0.9rem;">إجمالي: <span style="color:#fff; font-weight:bold;">${filtered.length}</span></span>
                    <select onchange="changeVioRows(this.value)" class="form-control" style="width:auto; padding:4px 8px; height:30px; font-size:0.85rem;">
                        <option value="50" ${vioRowsPerPage == 50 ? 'selected' : ''}>50 / صفحة</option>
                        <option value="100" ${vioRowsPerPage == 100 ? 'selected' : ''}>100 / صفحة</option>
                        <option value="200" ${vioRowsPerPage == 200 ? 'selected' : ''}>200 / صفحة</option>
                    </select>
                </div>
                <div style="display:flex; align-items:center; gap:10px;">
                    <button class="btn btn-secondary" onclick="changeVioPage(-1)" ${window.vioCurrentPage === 1 ? 'disabled' : ''} style="padding:4px 10px;">&lt; السابق</button>
                    <span style="color:#fff; font-size:0.9rem;">صفحة ${window.vioCurrentPage} من ${totalPages}</span>
                    <button class="btn btn-secondary" onclick="changeVioPage(1)" ${window.vioCurrentPage >= totalPages ? 'disabled' : ''} style="padding:4px 10px;">التالي &gt;</button>
                </div>
            </div>
        `;
    }
...
function changeVioRows(val) {
    vioRowsPerPage = parseInt(val);
    window.vioCurrentPage = 1;
    renderViolationsEditor();
}

function changeVioPage(delta) {
    window.vioCurrentPage += delta;
    renderViolationsEditor();
}
...
let licRowsPerPage = 50;

function renderLicensesTable() {
    const tbody = document.getElementById('licenses-table-body');
    const paginationEl = document.getElementById('lic-pagination');
    if(!tbody) return;

    const data = safeParse('admin_licenses', []);
    
    // Pagination State
    if (!window.licCurrentPage) window.licCurrentPage = 1;
    ...
    const totalPages = Math.ceil(totalRows / licRowsPerPage) || 1;
    if (window.licCurrentPage > totalPages) window.licCurrentPage = totalPages;
    if (window.licCurrentPage < 1) window.licCurrentPage = 1;

    const startIndex = (window.licCurrentPage - 1) * licRowsPerPage;
    const endIndex = Math.min(startIndex + licRowsPerPage, totalRows);
    const displayedRows = data.slice(startIndex, endIndex);

    tbody.innerHTML = '';

    displayedRows.forEach((item, relativeIndex) => {
        // Calculate actual index for editing/deleting
    ...
    if (paginationEl) {
        paginationEl.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                <div style="display:flex; align-items:center; gap:15px;">
                    <span style="color:#94a3b8; font-size:0.9rem;">إجمالي: <span style="color:#fff; font-weight:bold;">${totalRows}</span></span>
                    <select onchange="changeLicRows(this.value)" class="form-control" style="width:auto; padding:4px 8px; height:30px; font-size:0.85rem;">
                        <option value="50" ${licRowsPerPage == 50 ? 'selected' : ''}>50 / صفحة</option>
                        <option value="100" ${licRowsPerPage == 100 ? 'selected' : ''}>100 / صفحة</option>
                        <option value="200" ${licRowsPerPage == 200 ? 'selected' : ''}>200 / صفحة</option>
                    </select>
                </div>
                <div style="display:flex; align-items:center; gap:10px;">
                    <button class="btn btn-secondary" ${window.licCurrentPage === 1 ? 'disabled' : ''} onclick="changeLicPage(-1)" style="padding:4px 10px;">&lt; السابق</button>
                    <span style="color:#fff; font-size:0.9rem;">صفحة ${window.licCurrentPage} من ${totalPages}</span>
                    <button class="btn btn-secondary" ${window.licCurrentPage === totalPages ? 'disabled' : ''} onclick="changeLicPage(1)" style="padding:4px 10px;">التالي &gt;</button>
                </div>
            </div>
        `;
    }
...
function changeLicPage(delta) {
    window.licCurrentPage += delta;
    renderLicensesTable();
}

function changeLicRows(val) {
    licRowsPerPage = parseInt(val);
    window.licCurrentPage = 1;
    renderLicensesTable();
}
...
function openLicenseModal(index = -1) {
    ...
    if(index === -1) {
        ...
        document.getElementById('lic-branch-file').value = ''; // Reset file input
        document.getElementById('lic-branch-file-view').innerHTML = ''; // Reset preview
        ...
        document.getElementById('lic-out-file').value = '';
        document.getElementById('lic-out-file-view').innerHTML = '';
...
async function saveLicense() {
    try {
        ...
        // --- Branch General File (NEW) ---
        let branchFile = existingItem?.branch_file || '';
        const branchFileInput = document.getElementById('lic-branch-file');
        if(branchFileInput && branchFileInput.files && branchFileInput.files[0]) {
            // Upload logic (Firebase or Base64)
            const fbUrl = await uploadFileToFirebase(branchFileInput.files[0], 'licenses/branch_files');
            branchFile = fbUrl || await readFileAsBase64(branchFileInput.files[0]);
        }
        ...
        const item = {
            ...
            branch_file: branchFile, // Save file path/url
            ...
            // --- Outdoor Permit File ---
            permit_out: (function() {
                // We need to handle async upload here, but this object creation is sync.
                // Refactoring slightly to handle upload before object creation or use temp variable
                return {
        ...
        // --- Outdoor Permit File Upload ---
        let outFile = existingItem?.permit_out?.file || '';
        const outFileInput = document.getElementById('lic-out-file');
        if(outFileInput && outFileInput.files && outFileInput.files[0]) {
            const fbUrl = await uploadFileToFirebase(outFileInput.files[0], 'licenses/outdoor');
            outFile = fbUrl || await readFileAsBase64(outFileInput.files[0]);
        }
        item.permit_out.file = outFile;
...
// Initialize Search Listener for Employees
document.addEventListener('DOMContentLoaded', () => {
